...
const arrowSum = _(nums).reduce((acc,item) => acc+item); // nums.reduce
...
if(harpist)
{
  console.log(`The harpist is: ${harpist.name}. Instrument played: ${harpist.instrument}, Age: ${harpist.age}`);
}
...
const filteredNums = nums.filter(function(item) {
  return item > 3;
});

console.log(filteredNums);

// Chaining array methods: the output array produced by filter
// first narrow down the list of brothers to only the ones with nore than 3 favorite numbers,
// then instead of dealing with an array of obnjects, use .map to just get an array of the name
...
// Which brothers have the number 5 in their array of favorite numbers?
